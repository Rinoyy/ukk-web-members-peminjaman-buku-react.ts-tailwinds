...
import { BookMarked, ClipboardList, User, ArrowRight, type LucideIcon } from 'lucide-react';

const Home = () => {
    const { user } = useAuth();

    const features: { icon: LucideIcon; title: string; desc: string; link: string }[] = [
        { icon: BookMarked,    title: 'Jelajahi Buku',   desc: 'Temukan koleksi buku terlengkap',    link: '/books' },
        { icon: ClipboardList, title: 'Riwayat Pinjam',  desc: 'Lihat status peminjaman Anda',       link: '/history' },
        { icon: User,          title: 'Profil Saya',     desc: 'Lihat QR Code untuk check-in',       link: '/profile' },
    ];
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                {features.map((f, idx) => (
                    ...
                    >
                        <f.icon className="w-10 h-10 mb-4 text-gray-700 group-hover:scale-110 transition-transform" />
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        ...
                        >
                            Lihat QR lebih besar <ArrowRight className="w-3 h-3" />
                        </Link>
                    </div>
                </div>
            </div>
